...
def estimate_s1ab(frame_esds, col = 'daz_bovl_notide_noiono', rmsiter = 50, printout = True):
...
def estimate_s1ab_allframes(esds, framespd, col = 'daz_bovl_notide_noiono', rmsiter = 50):
    lenframes = len(framespd['swath_overlap'])
    ...
    i = 0
    for swath_overlap in framespd['swath_overlap']:
        i=i+1
        print('  Running for {0:6}/{1:6}th swath...'.format(i, lenframes), flush=True, end='\r')
        frameta = framespd[framespd['swath_overlap']==swath_overlap].copy()
        selected_frame_esds = esds[esds['swath_overlap'] == swath_overlap].copy()
...
def correct_s1ab(esds, framespd, cols = ['daz_mm', 'daz_mm_notide', 'daz_bovl_notide_noiono'], stderr_thres = 100):
    '''
...
def df_calculate_slopes(esdsin, framespdin, alpha = 2.5, eps = 1.5, bycol = 'daz_bovl_notide_noiono', subset = True, roll_assist = False):
    ...
    for swath_overlap, group in esds.groupby('swath_overlap'):
        ...
        if swath_overlaps.empty:
            print('swath_overlap data is empty, skipping')
            continue
        # make preselection
        grsel = group.copy(deep=True)
        grsel = grsel[np.isfinite(grsel[bycol])]

        # Ensure 'epoch' is in datetime format
        if not np.issubdtype(grsel['epoch'].dtype, np.datetime64):
            grsel['epoch'] = pd.to_datetime(grsel['epoch'])
        # Debug statement to check the 'epoch' column before filtering 
        if subset:
            #limiting the dataset here, as often data before mid-2016 are influenced by ionosphere (probably)
            grsel = grsel[grsel['epoch'] > pd.Timestamp('2017-01-01')]
            
        if len(grsel) < 15:
            print('most of data removed - cancelling for this frame')
            continue
        correctback = np.median(grsel['daz_bovl'])
        grsel['daz_bovl'] = grsel['daz_bovl'] - correctback #np.median(grsel['daz_mm'])
        grsel = grsel[grsel['daz_bovl'] > -400][grsel['daz_bovl'] < 400]
        if len(grsel) < 15:
            print('most of data removed - cancelling for this frame')
            continue
        ...
        #print(swath_overlaps)
        colwarm = 'slope_plates_vel_azi_gps'
        if not colwarm in swath_overlaps:
            colwarm = 'slope_plates_vel_azi_itrf2014'
        itrfslope = swath_overlaps[colwarm].values[0]
        ...
        try:
            ...
            rmse = np.nan
        #
        framespd.at[swath_overlaps.index.values[0], 'slope_'+bycol+'_mmyear'] = slope
        framespd.at[swath_overlaps.index.values[0], 'intercept_'+bycol+'_mmyear'] = intercept + correctback
        # add the rest:
        #keeping the older approach, but perhaps not needed...?
        residuals = y - y_pred
        framespd.at[swath_overlaps.index.values[0], bycol+'_RMSE_selection'] = rmse
        framespd.at[swath_overlaps.index.values[0], bycol+'_count_selection'] = len(residuals)
        framespd.at[swath_overlaps.index.values[0], bycol+'_RMSE_full'] = rmse_full
        # the STD mmy is perhaps the best estimate - kudos to Andy Hooper for this error propagation calculation approach
        framespd.at[swath_overlaps.index.values[0], bycol+'_RMSE_mmy_full'] = std_vel_full
        #
        ...
        framespd.at[swath_overlaps.index.values[0], bycol+'_RMSE_mmy_full_error_multi'] = rms_vel_full
